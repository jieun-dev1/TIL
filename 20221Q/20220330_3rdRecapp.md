...
Mark&Sweep 은 Old 영역에서도 일어날까? 

일어난다. 다만 GC 의 종류마다, Old 영역 GC의(Major GC) 알고리즘은 조금씩 다르다. 
Serial GC 에서는 Mark, Sweep, Compact 알고리즘을 사용하고, 
Parallel GC 는 알고리즘은 같지만 멀티쓰레드에서 실행한다 (Serial 은 싱글 스레드에서 실행한다. 데스크톱의 
cpu 가 하나만 있을 때 사용하려고 만들어진 방식이라서 운영 서버에서는 권장되지 않는다.)
Parallel Old GC 는 Mark-Summary-Compaction 이다. Summary 단계는 
앞서 GC 를 수행한 영역에 대해서 별도로 살아있는 객체를 식별한다는 점에서 Sweep 보다는 약간 더 복잡하다. 
CMS GC는 Compact 이 없이 3번의 Mark-Sweep 까지만 진행한다. 
G1GC 
...
Stop The world의 단점은 collection cycle이 진행되는 동안 프로그램이 다른 일을 할 수 ㅓㅄ다는 것이다. 
